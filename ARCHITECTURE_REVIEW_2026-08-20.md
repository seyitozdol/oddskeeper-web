...
**P-1. FS kupa yolu H2 kapısını deliyor (ölçüldü, en büyük kalem).** *DURUM: KAPANDI (2026-08-20 öğleden sonra, commit 5622e29, VPS deploy edildi). FS lig ve kupa fetcher'ları sofa'daki hash desenini koşuyor (FS_CHANGED_M / CUP_FS_CHANGED_M satırları), wrapper kapıları değişiklik-bazlı ve fail-open; upsert davranışı değişmedi, kapı mantığı 7 senaryoluk simülasyonla test edildi. Canlı teyit 20 Ağu akşam EL turlarında (tekrar turlarda CUP MAT banner'ı görünmemeli).* 19-20 Ağu gecesi aynı 4 CL maçı FlashScore grace penceresi boyunca her 10 dk turunda yeniden upsert edildi ve kupa mat'ları 22 ARDIŞIK turda tazelendi (21:35'ten 00:54'e "CUP MAT OK" 22 banner; H2 SKIP olan turlarda bile). H2 Faz 2'de bilinçli açık bırakılan "FlashScore yolu tuzağı" artık ölçülmüş maliyet. Düzeltme: fetch_flashscore_cup_matches'e (ve FS 1.Lig yoluna) sofascore'daki payload-hash kapısının aynısı; değişiklik yoksa cup_islendi tetiklenmesin. Kazanç: maç akşamı kupa refresh yükünün ~%80-90'ı. Emek: S-M.

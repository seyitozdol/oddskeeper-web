...
// Avrupa kupalari kopyasi: app/dashboard/tff-1-lig/player-market/list-tabs.tsx
// ile birebir ayni UI. Tek fark parametrikleştirme: sekmeler `league` prop'u
// ("eurocl" | "euel" | "euecl") alir ve ./queries cagrilarina gecirir
// (eurocup_* view'lari + pm_* tablolarinda league ayrimi). Oyuncu Listesi'nde
// ulke kolonu yok (tff1 ile ayni), logo/ID anahtarlari team_id/player_id
// (sofascore).
...
// ─── Player List tab ──────────────────────────────────────────────────────────
// Guncel kadrolardaki tum oyuncular; ID kolonu pm_player_ids'e (league=prop)
// kaydedilir, anahtar sofascore player_id'dir.
...
export function PlayerListTab({
  ...
  useEffect(() => {
    Promise.all([fetchAllCurrentPlayers(league), fetchPlayerIds(league)]).then(
      ([p, storedIds]) => {
        setPlayers(p);
        setIds(storedIds);
        setLoading(false);
  ...
  async function handleSave() {
    setSaving(true);
    const ok = await savePlayerIds(league, ids);
    setSaving(false);
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto rounded-lg border border-line">
          <table className="min-w-full border-collapse text-[12px]">
            ...
            <tbody>
              {filtered.map((p) => {
                const logo = p.team_id ? teamLogos[p.team_id] : undefined;
                return (
                  <tr key={p.player_id} className="border-t border-line transition hover:bg-veil">
                    ...
                    <td className="px-2 py-1.5 text-ink-3">{p.position ?? "—"}</td>
                    <td className="px-2 py-1.5">
                      <input
                        type="text"
                        value={ids[p.player_id] ?? ""}
                        onChange={(e) =>
...
// ve ozel marketler analytics.pm_markets'a (league=prop) kaydedilir.
...
export function MarketListTab({
  ...
  async function handleSaveAll() {
    ...
    for (const row of rows) {
      ...
      if ((stored?.template_id ?? "") === value && (stored?.market_type ?? "static") === type)
        continue; // degismedi
      const res = await upsertStoredMarket(league, {
        market_key: row.key,
        label: row.label,
  ...
  // upsert edilir; kayitli template/tur/sort korunur.
  async function toggleInModel(
    row: { key: string; label: string; isCustom: boolean },
    next: boolean
  ) {
    const stored = storedByKey.get(row.key);
    const ok = await upsertStoredMarket(league, {
      market_key: row.key,
      label: row.label,
  ...
  async function handleDelete(key: string) {
    const ok = await deleteStoredMarket(league, key);
    if (ok) {
      setTemplateIds((prev) => {
        const next = { ...prev };
        delete next[key];
...
// ─── Input tab ────────────────────────────────────────────────────────────────
// TSL ile ayni: Model'de Ekle'ye basilinca uretilen satirlar, secili marketin
// turune gore Static Input veya Dynamic Input segmentine duser. Yazdir tek
// sheet'li ("input") xlsx uretir, dosya adi fixture adidir.
...
export type StaticInputRow = {
  ...
  playerSlug: string; // = sofascore player_id
  playerName: string;
...
// (league=prop) yazar; Model'deki Ekle akisi bu kayitlari kullanir.

export function FixtureIdTab({
  league,
  fixtures,
}: {
  league: EuroCupLeague;
  ...
  useEffect(() => {
    fetchFixtureInputs(league).then((v) => {
      setValues(v);
      setLoading(false);
    });
    fetchBets10FixtureIds(league).then(setLinks);
  }, [league]);
  ...
  async function handleSave() {
    setSaving(true);
    const ok = await saveFixtureInputs(league, values);
    setSaving(false);
    if (ok) setSavedAt(Date.now());
  }

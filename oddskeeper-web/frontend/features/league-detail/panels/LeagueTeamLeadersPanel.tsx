...
function getMetricDefinition(
  ...
  if (key.includes("pass_accuracy")) {
    titleText = "Ball circulation quality";
    text = "Share of completed passes. Higher values indicate cleaner and more controlled build-up.";
  } else if (key.includes("accurate_pass")) {
    titleText = "Completed pass volume";
    text = "Completed pass output generated by each team.";
  } else if (key.includes("expected_goals")) {
    titleText = "Chance quality creation";
    text = "Expected goals measure the quality of chances created by each team.";
  } else if (key.includes("shots_on_target")) {
    titleText = "On-target threat";
    text = "Shots on target reflect clean final-third execution and testing the goalkeeper.";
  } else if (key.includes("goals_against")) {
    titleText = "Defensive control";
    text = "Goals conceded show how well a team protects its defensive floor.";
  } else if (key.includes("goals_for")) {
    titleText = "Final-third output";
    text = "Goals scored reflect end-product and conversion at league level.";
  } else if (key.includes("interceptions")) {
    titleText = "Passing-lane disruption";
    text = "Interceptions capture defensive anticipation and passing-lane control.";
  } else if (key.includes("tackles")) {
    titleText = "Direct defensive actions";
    text = "Tackles capture direct ball-winning challenges completed by the team.";
  } else if (key.includes("fouls_conceded")) {
    titleText = "Disciplinary control";
    text = "Fouls committed. Lower values usually indicate cleaner defending and better control.";
  } else if (key.includes("fouls_won")) {
    titleText = "Pressure and duel leverage";
    text = "Fouls won show how often a team draws contact and disrupts the opponent.";
  } else if (key.includes("yellow_cards")) {
    titleText = "Card discipline";
    text = "Yellow cards received. Lower values indicate cleaner discipline.";
  } else if (key.includes("red_cards")) {
    titleText = "Severe discipline risk";
    text = "Red cards received. Lower values indicate stronger discipline and control.";
  } else if (key.includes("passes")) {
    titleText = "Possession circulation";
    text = "Passing volume used to control possession and shape build-up phases.";
  } else if (key.includes("shots")) {
    titleText = "Shot generation";
...
function getDeltaSemanticText(
  ...
  if (better) {
    return `${formatted} better than ${suffix}`;
  }

  return `${formatted} behind ${suffix}`;
}
            ...
            <div className="inline-flex rounded-lg border border-white/10 bg-white/[0.03] p-1">
              ...
              <button
                ...
                title={
                  totalAllowed
                    ? "Show total values"
                    : "Total basis is not meaningful for this metric"
                }
              >
                Total
              </button>
          ...
          <thead className="sticky top-0 z-10 bg-[#0d1624]">
            <tr className="text-left text-[10px] uppercase tracking-[0.14em] text-white/38">
              ...
              <th className="px-4 py-2 font-medium cursor-pointer hover:text-white/70 select-none" onClick={() => handleColSort("value")}>
                {visibleBasis === "total" && totalAllowed ? "Total" : "Per Match"}<SortIcon col="value" />
              </th>
              ...
              const deltaText =
                ...
                Number.isNaN(row.vs_league_avg_pct)
                  ...
                  ? row.vs_league_avg_pct < 0
                    ? `${formatPct(Math.abs(row.vs_league_avg_pct))} better`
                    : `${formatPct(Math.abs(row.vs_league_avg_pct))} worse`
                  : row.vs_league_avg_pct > 0
                  ? `${formatPct(Math.abs(row.vs_league_avg_pct))} better`
                  : `${formatPct(Math.abs(row.vs_league_avg_pct))} worse`;

...
type ColumnDef<Row> = {
  ...
  labelKey: string;
  format?: NumFormat;
  // true: "Mac basina" modunda deger mac sayisina bolunur
  perMatch?: boolean;
};
...
const PLAYER_GROUP_COLUMNS: Record<MetricGroup, ColumnDef<PlayerRowX>[]> = {
  general: [
    ...
    { key: "minutes", labelKey: "tff1.colMinutes", perMatch: true },
    { key: "goals", labelKey: "tff1.colGoals", perMatch: true },
    { key: "assists", labelKey: "tff1.colAssists", perMatch: true },
    { key: "xg", labelKey: "tff1.colXg", format: "dec2", perMatch: true },
    { key: "yellow_cards", labelKey: "tff1.colYellowCards", perMatch: true },
    { key: "red_cards", labelKey: "tff1.colRedCards", perMatch: true },
  ...
  attack: [
    { key: "goals", labelKey: "tff1.colGoals", perMatch: true },
    { key: "xg", labelKey: "tff1.colXg", format: "dec2", perMatch: true },
    { key: "xgot", labelKey: "tff1.colXgot", format: "dec2", perMatch: true },
    { key: "assists", labelKey: "tff1.colAssists", perMatch: true },
    { key: "xa", labelKey: "tff1.colXa", format: "dec2", perMatch: true },
    { key: "shots", labelKey: "tff1.colShots", perMatch: true },
    { key: "shots_on_target", labelKey: "tff1.colShotsOnTarget", perMatch: true },
    { key: "big_chances_missed", labelKey: "tff1.colBigChancesMissed", perMatch: true },
    { key: "hit_woodwork", labelKey: "tff1.colHitWoodwork", perMatch: true },
    { key: "dribbles_won", labelKey: "tff1.colDribblesWon", perMatch: true },
    { key: "dribbles_attempted", labelKey: "tff1.colDribblesAttempted", perMatch: true },
    { key: "was_fouled", labelKey: "tff1.colWasFouled", perMatch: true },
    { key: "offsides", labelKey: "tff1.colOffsides", perMatch: true },
  ],
  passing: [
    { key: "total_passes", labelKey: "tff1.colPasses", perMatch: true },
    { key: "accurate_passes", labelKey: "tff1.colAccuratePasses", perMatch: true },
    { key: "pass_accuracy", labelKey: "tff1.colPassAccuracy", format: "pct" },
    { key: "key_passes", labelKey: "tff1.colKeyPasses", perMatch: true },
    { key: "big_chances_created", labelKey: "tff1.colBigChancesCreated", perMatch: true },
    { key: "crosses", labelKey: "tff1.colCrosses", perMatch: true },
    { key: "accurate_crosses", labelKey: "tff1.colAccurateCrosses", perMatch: true },
    { key: "long_balls", labelKey: "tff1.colLongBalls", perMatch: true },
    { key: "accurate_long_balls", labelKey: "tff1.colAccurateLongBalls", perMatch: true },
  ],
  defense: [
    { key: "tackles", labelKey: "tff1.colTackles", perMatch: true },
    { key: "tackles_won", labelKey: "tff1.colTacklesWon", perMatch: true },
    { key: "interceptions", labelKey: "tff1.colInterceptions", perMatch: true },
    { key: "clearances", labelKey: "tff1.colClearances", perMatch: true },
    { key: "blocks", labelKey: "tff1.colBlocks", perMatch: true },
    { key: "ball_recoveries", labelKey: "tff1.colBallRecoveries", perMatch: true },
    { key: "duels_won", labelKey: "tff1.colDuelsWon", perMatch: true },
    { key: "duels_lost", labelKey: "tff1.colDuelsLost", perMatch: true },
    { key: "aerials_won", labelKey: "tff1.colAerialsWon", perMatch: true },
    { key: "fouls", labelKey: "tff1.colFouls", perMatch: true },
  ],
  physical: [
    { key: "appearances", labelKey: "tff1.colAppearances" },
    { key: "minutes", labelKey: "tff1.colMinutes", perMatch: true },
    { key: "km_covered", labelKey: "tff1.colKmCovered", format: "dec1", perMatch: true },
    { key: "sprints", labelKey: "tff1.colSprints", perMatch: true },
    { key: "top_speed", labelKey: "tff1.colTopSpeed", format: "dec1" },
  ],
  keeper: [
    { key: "appearances", labelKey: "tff1.colAppearances" },
    { key: "minutes", labelKey: "tff1.colMinutes", perMatch: true },
    { key: "saves", labelKey: "tff1.colSaves", perMatch: true },
...
const TEAM_COLUMNS: ColumnDef<Tff1TeamRow>[] = [
  ...
  { key: "goals_for", labelKey: "tff1.colGoalsFor", perMatch: true },
  { key: "goals_against", labelKey: "tff1.colGoalsAgainst", perMatch: true },
  { key: "goal_diff", labelKey: "tff1.colGoalDiff" },
  { key: "points", labelKey: "tff1.colPoints", perMatch: true },
...
// "Mac basina" modunda sayim kolonlari mac sayisina bolunur; kolon formatlari 2 ondalik olur
function applyPerMatch<Row extends Record<string, unknown>>(
  rows: Row[],
  columns: ColumnDef<Row>[],
  matchesKey: keyof Row & string
): Row[] {
  const perMatchKeys = columns.filter((c) => c.perMatch).map((c) => c.key);
  if (perMatchKeys.length === 0) return rows;
  return rows.map((row) => {
    const matches = toNumber(row[matchesKey] as number | string | null);
    if (!matches) return row;
    const copy: Record<string, unknown> = { ...row };
    for (const key of perMatchKeys) {
      const v = toNumber(row[key] as number | string | null);
      copy[key] = v === null ? null : v / matches;
    }
    return copy as Row;
  });
}
...
function TeamBadge({ logoUrl, name }: { logoUrl?: string; name: string | null }) {
  if (logoUrl) {
    return (
  ...
  return (
    <span className="flex h-[18px] w-[18px] shrink-0 items-center justify-center rounded-full bg-veil text-[9px] font-semibold text-ink-3">
      {(name ?? "?").slice(0, 1)}
    </span>
  );
}
...
export default function Tff1Explorer({
  ...
  const visiblePlayers = useMemo(() => {
    ...
    if (valueMode === "perMatch") {
      rows = applyPerMatch(rows, playerColumns, "appearances");
    }
    return [...rows].sort((a, b) =>
      compare(a as Record<string, unknown>, b as Record<string, unknown>)
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [enrichedPlayers, season, search, group, valueMode, sortKey, sortAsc]);
  ...
  const visibleTeams = useMemo(() => {
    ...
    if (valueMode === "perMatch") {
      rows = applyPerMatch(rows, TEAM_COLUMNS, "played");
    }
    return [...rows].sort((a, b) =>
      compare(a as Record<string, unknown>, b as Record<string, unknown>)
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [teams, season, search, valueMode, sortKey, sortAsc]);
  ...
  const fmt = (col: ColumnDef<Record<string, unknown>>, value: unknown) => {
    if (valueMode === "perMatch" && col.perMatch) {
      return formatValue(value as number | null, "dec2");
    }
    return formatValue(value as number | null, col.format);
  };
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-1 rounded-xl border border-line bg-veil p-1">
          <button
            ...
            className={segmentClass(valueMode === "total")}
          >
            {t("tff1.valueModeTotal")}
          </button>
          <button
            type="button"
            onClick={() => setValueMode("perMatch")}
            className={segmentClass(valueMode === "perMatch")}
          >
            {t("tff1.valueModePerMatch")}
          </button>
        </div>
              ...
              <tbody>
                {visiblePlayers.map((row, index) => (
                  ...
                  >
                    ...
                    >
                      <Link
                        href={`/dashboard/tff-1-lig/player/${row.player_id}`}
                    ...
                    <td className="whitespace-nowrap px-3 py-2 text-ink-2">
                      <Link
                        href={`/dashboard/tff-1-lig/team/${row.team_id}?season=${encodeURIComponent(row.season_label)}`}
                        className="flex items-center gap-1.5 transition hover:text-accent-ink hover:underline"
                      >
                        <TeamBadge
                          logoUrl={row.team_id ? teamLogos[row.team_id] : undefined}
                          name={row.team_name}
                        />
                        {row.team_name ?? "—"}
                      </Link>
                  ...
                  >
                    ...
                    <td className="whitespace-nowrap px-3 py-2 font-medium">
                      <Link
                        ...
                        className="flex items-center gap-1.5 transition hover:text-accent-ink hover:underline"
                      >
                        <TeamBadge logoUrl={teamLogos[row.team_id]} name={row.team_name} />
                        {row.team_name ?? "—"}
                      </Link>
                    </td>
                    {TEAM_COLUMNS.map((col) => (

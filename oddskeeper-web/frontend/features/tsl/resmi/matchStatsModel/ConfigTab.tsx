...
export default function ConfigTab({
  ...
  const [section, setSection] = useState<"general" | "markets" | "stdHalves">("general");
  ...
  useEffect(() => {
    ...
    // Dişliden gelen odak: Markets kendi alt-sekmesinde, kalanlar Genel'de.
    setSection(focus === "markets" ? "markets" : "general");
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-1 rounded-lg border border-line bg-card-2 p-0.5">
          {(["general", "markets", "stdHalves"] as const).map((s) => (
            <button
              key={s}
      ...
      {/* Markets: market başına genel bayraklar + FT/1H/2H derin kontrol
          (çizgi sayısı, Under açık/kapalı, yarı payback'leri). */}
      {section === "markets" && (
        <section id="cfg-markets" className="scroll-mt-4 rounded-xl border border-line bg-card p-4">
          <h3 className="mb-1 text-sm font-semibold text-ink">{t("msm.cfgMarkets")}</h3>
          <p className="mb-3 max-w-3xl text-[11px] leading-relaxed text-ink-3">{t("msm.marketsNote")}</p>

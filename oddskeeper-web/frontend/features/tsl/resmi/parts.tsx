...
import { tslMatchHref } from "@/features/tsl/constants";
...
export function PlayerNameLink({
  ...
  const href = slug ? getPlayerDetailHref(slug) : null;
  if (href) {
    return (
        ...
        <table className="w-full text-[13px]">
          <thead>
            <tr className="border-b border-line text-[10px] uppercase tracking-[0.06em] text-ink-3">
              ...
              {!compact ? <th className="hidden px-2 py-2 text-left font-medium sm:table-cell">{labels.form}</th> : null}
...
// Tek mac satiri (sonuc veya fikstur), mac detayina link.
export function MatchRow({
  ...
  return (
    <Link href={tslMatchHref(match.matchId, returnTo)} className="block">
      {inner}
    </Link>
  );
}

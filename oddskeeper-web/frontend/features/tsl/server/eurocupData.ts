// Avrupa kupasi (CL/UL/Con) veri saglayici FABRIKASI. Tek fonksiyon, competition +
...
// Fabrika: competition (or. 'UEFA Avrupa Ligi') + view prefix (or. 'uel').
export function makeCupProvider(COMP: string, prefix: string) {
  const V = (name: string) => `${prefix}_${name}`;
  ...
  async function standings(season: string, meta: Record<string, TslTeamMeta>, matchList: TslMatch[]): Promise<TslStandingRow[]> {
    const rows = (await teamStatRows(season)).map((r) => {
      ...
      return {
        ...
        ppg: played > 0 ? points / played : 0, form: formOf(id, matchList),
  ...
  async function teamMetrics(season: string, meta: Record<string, TslTeamMeta>): Promise<TslTeamMetric[]> {
  ...
  async function teamLeaderboard(season: string, meta: Record<string, TslTeamMeta>): Promise<TslTeamLeaderRow[]> {
    const metrics = await teamMetrics(season, meta);
  ...
  async function aggression(season: string): Promise<Record<string, TeamAggression>> {
    const rows = await playerRows(season);
  ...
  return {
    teamMeta: () => teamMeta(),
    matches: (s: string, meta: Record<string, TslTeamMeta>) => matches(s, meta),
    ...
    teamLeaderboard: (s: string, meta: Record<string, TslTeamMeta>) => teamLeaderboard(s, meta),
    aggression: (s: string) => aggression(s),

...
def safe_parse_page(
    ...
    initial_status = detect_page_status(driver, page_type)
    if initial_status["status"] != "ok":
        logging.warning(
            f"{match_id} | {page_type} | özel durum bulundu: "
            f"{initial_status['status']} | eşleşme: {initial_status['matched_text']}"
        )
        ...
        return build_empty_page_result(
            ...
            page_status=initial_status
        )

    try:
        wait_func(driver)
    except Exception as e:
        status_after_wait = detect_page_status(driver, page_type)
        logging.warning(
            f"{match_id} | {page_type} | widget bekleme başarısız: "
            f"{type(e).__name__}: {e}"
        )
        save_debug_html(driver, output_dir, match_id, page_name)
        return build_empty_page_result(
            ...
            driver=driver,
            page_type=page_type,
            reason=f"widget_wait_failed: {type(e).__name__}",
            page_status=status_after_wait
        )
    ...
    try:
        ...
        status_after_parse = detect_page_status(driver, page_type)
        logging.warning(
            f"{match_id} | {page_type} | parse başarısız: "
            f"{type(e).__name__}: {e}"
        )
        save_debug_html(driver, output_dir, match_id, page_name)
        return build_empty_page_result(
            ...
            page_status=status_after_parse
        )
...
def wait_for_match_summary_widgets(driver):
    WebDriverWait(driver, 20).until(EC.presence_of_element_located((By.ID, "Opta_0")))
    WebDriverWait(driver, 20).until(EC.presence_of_element_located((By.ID, "Opta_1")))
    WebDriverWait(driver, 20).until(EC.presence_of_element_located((By.ID, "Opta_2")))
    time.sleep(1)
...
def build_incident_key(side: str, event_type_code, minute_text: str, player_texts: list[str]):
    minute_part = re.sub(r"[^0-9a-zA-Z+]", "", clean_text(minute_text)) or "nominute"
    event_part = clean_text(str(event_type_code or "noevent")).lower() or "noevent"
    player_part = "_".join(
        re.sub(r"[^0-9a-zA-ZçğıöşüÇĞİÖŞÜ]+", "", clean_text(p)).lower()
        for p in player_texts[:2]
        if clean_text(p)
    )
    if not player_part:
        player_part = "noplayer"
    return f"{side}-{event_part}-{minute_part}-{player_part}"
...
def parse_summary_incident_item(li_element, side: str):
    ...
    return {
        ...
        "incident_key": build_incident_key(side, event_type_code, minute_text, player_texts),

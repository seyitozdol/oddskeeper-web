...
STEPS = [
    {
        "name": "parse",
        "script": "parse_backup2_integrated_v6.py",
...
def decide_final_status(step_results: List[StepResult], run_log_ok: bool) -> str:
    ...
    failed_steps = [s for s in enabled_steps if s.status == "failed"]
    warning_steps = [s for s in enabled_steps if s.status == "warning"]
    unexpected_skipped = [s for s in enabled_steps if s.status == "skipped"]
...
def main() -> None:
    ...
    for step in STEPS:
        ...
        completed_steps = sum(1 for s in step_results if s.enabled and s.status == "success")
        warning_steps = sum(1 for s in step_results if s.enabled and s.status == "warning")
        failed_steps = sum(1 for s in step_results if s.enabled and s.status == "failed")
        ...
        if result.status == "failed" and result.critical and not CONTINUE_ON_CRITICAL_FAILURE:
    ...
    warning_steps = sum(1 for s in step_results if s.enabled and s.status == "warning")
    failed_steps = sum(1 for s in step_results if s.enabled and s.status == "failed")
    skipped_disabled = sum(1 for s in step_results if not s.enabled and s.status == "skipped")
    ...
    logging.info("=" * 60)
    logging.info("MASTER PIPELINE V3 ÖZETİ")

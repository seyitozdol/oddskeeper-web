...
def wait_for_common_match_shell(driver, timeout: int = 30):
    def shell_ready(d):
        try:
            selectors = [
...
def wait_for_match_summary_widgets(driver):
    WebDriverWait(driver, 35).until(EC.presence_of_element_located((By.ID, "Opta_0")))
    WebDriverWait(driver, 35).until(
        lambda d: d.find_elements(By.CSS_SELECTOR, "#Opta_0 .Opta-Cf[data-match]")
        or d.find_elements(By.CSS_SELECTOR, ".Opta-Cf[data-match]")
    )
    WebDriverWait(driver, 35).until(EC.presence_of_element_located((By.ID, "Opta_1")))
    WebDriverWait(driver, 35).until(EC.presence_of_element_located((By.ID, "Opta_2")))
    time.sleep(1.6)
...
def extract_match_info_from_opta0(driver, page_type: str):
    ...
    if not meta_candidates:
        raise NoSuchElementException(".Opta-Cf[data-match] bulunamadı")
    meta_block = meta_candidates[0]

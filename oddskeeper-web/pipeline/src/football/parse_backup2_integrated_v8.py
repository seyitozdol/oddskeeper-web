...
def create_driver():
    ...
    if (not HEADLESS_MODE) and MINIMIZE_WINDOW:
        try:
...
def wait_for_common_match_shell(driver, timeout: int = 45):
    def shell_ready(d):
        try:
            selectors = [
                "#Opta_0",
                "#Opta_0 td.Opta-TeamName",
                "#Opta_0 span.Opta-Team-Score",
                "#Opta_0 .Opta-Matchdata",
...
def wait_for_match_summary_widgets(driver):
    WebDriverWait(driver, 45).until(EC.presence_of_element_located((By.ID, "Opta_0")))
    WebDriverWait(driver, 45).until(
        lambda d: d.find_elements(By.CSS_SELECTOR, "#Opta_0 td.Opta-TeamName")
        or d.find_elements(By.CSS_SELECTOR, "#Opta_0 span.Opta-Team-Score")
        or d.find_elements(By.CSS_SELECTOR, "#Opta_0 .Opta-Matchdata")
        or d.find_elements(By.CSS_SELECTOR, "#Opta_0 .Opta-Cf[data-match]")
        or d.find_elements(By.CSS_SELECTOR, ".Opta-Cf[data-match]")
    )
    WebDriverWait(driver, 45).until(EC.presence_of_element_located((By.ID, "Opta_1")))
    WebDriverWait(driver, 45).until(EC.presence_of_element_located((By.ID, "Opta_2")))
    time.sleep(2.0)
...
def extract_match_info_from_opta0(driver, page_type: str):
    ...
    meta_candidates = container.find_elements(By.CSS_SELECTOR, ".Opta-Cf[data-match]")
    if not meta_candidates:
        meta_candidates = driver.find_elements(By.CSS_SELECTOR, ".Opta-Cf[data-match]")

    meta_block = meta_candidates[0] if meta_candidates else None
    match_id = meta_block.get_attribute("data-match") if meta_block else ""
    raw_date_ms = meta_block.get_attribute("data-date") if meta_block else ""
    winner_id = meta_block.get_attribute("data-match_winner_id") if meta_block else ""
    winner_side = meta_block.get_attribute("data-match_winner_side") if meta_block else ""
    ...
    if not match_id:
        m = re.search(r"/([a-z0-9]{10,})/(?:match-summary|opta-points|match-details)", driver.current_url, re.IGNORECASE)
        if m:
            match_id = m.group(1)
...
def wait_for_match_details_widget(driver):
    ...
    WebDriverWait(driver, 45).until(
        lambda d: d.find_elements(By.CSS_SELECTOR, "playerstats-match table.Opta-Striped")
        or d.find_elements(By.CSS_SELECTOR, "playerstats-match table")
    )
    time.sleep(2.0)

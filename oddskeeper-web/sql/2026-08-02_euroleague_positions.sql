-- EL/EC oyuncu pozisyonu okuma view'larına taşınır.
-- KAYNAK: euroleague.players.position_name (EuroLeague API'den ZATEN dolu:
--   Guard / Forward / Center — 3 kademe). BSL SofaScore (G/GF/F/FC/C) ile kıyas
--   için frontend normalizePositionCode "Guard"→G/"Forward"→F/"Center"→C indirir.
-- euroleague.players (comp,season,person) PK → 1:1 join, fan-out yok.

-- 1) el_player_season_v1: position eklendi (image_url join'ine bindirildi).
...
select
  ...
  max(pl.position_name)                                 as position,   -- SONA eklendi
...
-- 2) bsl_player_euro_seasons_v1: s.* → yeniden yaratınca position/height_cm otomatik gelir.
...
-- 3) el_player_leaderboard_v1: position eklendi (euro-only oyuncu profili + leaderboard).
...
select
  ...
  pl.image_url,
  pl.position_name as position,   -- SONA eklendi

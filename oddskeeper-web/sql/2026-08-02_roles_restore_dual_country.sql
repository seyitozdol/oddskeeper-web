-- 1) bb_player_role_v1: v5 (euro_focus + newcomer + merge-aware) RESTORE edilir
--    (roles_photo.sql yanlislikla temel surumle ezmisti) + sofascore_player_id eklenir.
-- 2) Ikili vatandaslik: cross-lig oyuncularin DIGER lig ulkesi (country_code2).
--    BSL season/leaderboard -> euro ulkesi; EL leaderboard -> BSL ulkesi.

-- ============================================================
-- 1) bb_player_role_v1 = v5 + sofascore_player_id
-- ============================================================
...
-- ============================================================
-- 2) bb_player_season_stats_v1 = country surumu + country_code2 (euro ulkesi)
-- ============================================================
...
select
  ...
  (select p2.country_code from euroleague.player_bsl_link l
     join euroleague.players p2 on p2.person_code = l.person_code
     left join analytics.bb_pm_player_merges mm
       on mm.league = 'basketball' and mm.alias_slug = l.bsl_player_slug
     where coalesce(mm.canonical_slug, l.bsl_player_slug) = agg.player_slug
       and p2.country_code is not null
     order by p2.season_code desc limit 1) as country_code2
from agg;

grant select on analytics.bb_player_season_stats_v1 to anon, authenticated;

-- ============================================================
-- 3) bb_player_leaderboard_v1: DROP+CREATE -> country_code2 gelir
-- ============================================================
...
-- ============================================================
-- 4) el_player_leaderboard_v1: country_code2 (BSL ulkesi) ekle (role+crest+country korunur)
-- ============================================================
...
select
  ...
  tm.crest_url,
  pl.country_code,
  (select bp.country_code from basketball.players bp
     where bp.player_slug = l.bsl_player_slug and bp.country_code is not null limit 1) as country_code2
from agg a

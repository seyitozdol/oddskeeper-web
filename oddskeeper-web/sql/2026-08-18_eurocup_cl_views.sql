-- 2026-08-18 Faz 4: Sampiyonlar Ligi frontend view'lari. tff1_* (1.Lig,
-- sofascore-keyed) tanimlarindan klonlanip competition literali
-- 'Trendyol 1. Lig' -> 'UEFA Şampiyonlar Ligi' cevrildi. ucl_* plain view
-- (CL ~357 mac; mat gerekmiyor). player_info/team_logos generic (id-bazli)
-- tff1_player_info_v1/tff1_team_logos_v1 dogrudan yeniden kullanilir.

CREATE OR REPLACE VIEW analytics.ucl_matches_v1 AS
 SELECT season_label,
  ...
  WHERE source = 'sofascore'::text AND (competition = ANY (ARRAY['UEFA Şampiyonlar Ligi'::text, 'UEFA Şampiyonlar Ligi Play-off'::text]));

CREATE OR REPLACE VIEW analytics.ucl_fixtures_v1 AS
 SELECT fixture_id,
    season_label,
    competition,
    round_number,
  ...
  WHERE source = 'sofascore'::text AND competition = 'UEFA Şampiyonlar Ligi'::text;
...
-- Frontend anon/authenticated rolu okuyabilsin (anon-grant tuzagi).
GRANT SELECT ON analytics.ucl_matches_v1 TO anon, authenticated;
GRANT SELECT ON analytics.ucl_fixtures_v1 TO anon, authenticated;
GRANT SELECT ON analytics.ucl_player_season_stats_v1 TO anon, authenticated;
GRANT SELECT ON analytics.ucl_team_season_stats_v1 TO anon, authenticated;

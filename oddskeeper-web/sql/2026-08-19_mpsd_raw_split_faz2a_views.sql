-- 2026-08-19: mpsd raw_stats yan tablo ayrimi FAZ 2 / ADIM A (okuyucu gecisi)
--
-- 22 nesne raw_stats'i artik yan tablodan (football.match_player_stats_raw)
-- okur. Yontem: football.mpsd_with_raw compat view'i (mpsd LEFT JOIN yan tablo,
-- dogal anahtar uzerinden 1:1) kurulur ve nesnelerdeki mpsd referansi ona
-- cevrilir. Kolon sozlesmeleri DEGISMEZ (view'lar CREATE OR REPLACE ile atomik
-- degisir; matview'ler rename-swap ile).
--
-- Tanimlar pg_get_viewdef ciktisindan URETILDI (elle yazim hatasi riski yok);
-- tek fark football.match_player_stats_details -> football.mpsd_with_raw.
--
-- Bu adim tek basina DB'yi kucultmez; kolon drop'u ADIM C'de (writer gecisi
-- kanitlandiktan sonra). Geri alma: ROLLBACK dosyasi (ayni tanimlar, eski
-- referansla).

-- 1) Compat view: mpsd (sicak kolonlar) + yan tablodan raw_stats.
--    ACIK kolon listesi kullanildi: ADIM C'de mpsd.raw_stats dusunce 'd.*'
--    davranisi degismesin ve kolon sozlesmesi sabit kalsin.
create or replace view football.mpsd_with_raw as
select
  d.id,
  d.source,
  d.source_match_id,
...
left join football.match_player_stats_raw r
 ...
 and r.source_match_id = d.source_match_id
 and r.source_player_id = d.source_player_id;

comment on view football.mpsd_with_raw is
  'mpsd sicak kolonlar + yan tablodan raw_stats (Faz 2, 2026-08-19). raw_stats okuyan tum view/mat buradan besleniyor.';

-- 2) Normal view'lar: CREATE OR REPLACE (atomik, kolon sozlesmesi ayni).

-- analytics._af_match_player_v1  (1 referans)
...
-- analytics.eurocup_fs_player_match_log_v1  (1 referans)
...
-- analytics.eurocup_player_match_log_v1  (1 referans)
...
-- analytics.player_current_info_bridged_def_v1  (1 referans)
...
-- analytics.player_match_log_sofascore_def_v1  (4 referans)
...
-- analytics.player_match_metrics_base_v1  (1 referans)
         ...
         SELECT COALESCE(o.source_match_id, d.source_match_id) AS source_match_id,
           ...
           FROM football.match_player_stats_opta_points o
             FULL JOIN ( SELECT match_player_stats_details.id,
                   ...
                   FROM football.mpsd_with_raw match_player_stats_details
...
-- analytics.player_profile_sofascore_v1  (2 referans)
...
-- analytics.tff1_player_match_log_v1  (1 referans)
...
-- analytics.tff1_player_season_stats_v1  (2 referans)
...
-- analytics.tff1_team_season_stats_v1  (1 referans)
...
-- analytics.tsl_player_advanced_season_v1  (2 referans)
...
-- analytics.tsl_player_flashscore_season_v1  (1 referans)
...
-- analytics.tsl_ss_player_detailed_metrics_v1  (1 referans)
...
-- analytics.tsl_ss_team_detailed_metrics_v1  (1 referans)
...
-- analytics.ucl_team_season_stats_v1  (1 referans)
...
-- analytics.uecl_team_season_stats_v1  (1 referans)
...
-- analytics.uel_team_season_stats_v1  (1 referans)
...
-- analytics.v_player_match_details_base  (1 referans)
